...
class LogisticModule(BaseModule):
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        exp_part = np.exp(X @ self.weights) / (1 + np.exp(X @ self.weights))
        diff = y - exp_part
...
class RegularizedModule(BaseModule):
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        if self.include_intercept_:
            return self.fidelity_module_.compute_jacobian(**kwargs) + self.lam_ * np.concatenate((np.array([0]), self.regularization_module_.compute_jacobian(**kwargs)))
        else:
